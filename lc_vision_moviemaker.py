...
# A brand-new node launches on this preset (rather than Custom, whose
# fresh/empty custom_system_prompt used to hard-error on first run) and it's
# also what Custom silently falls back to if custom_system_prompt is empty --
# no reference images needed, so it always works regardless of what's wired.
FALLBACK_PRESET = "Minimax H3 T2V"
...
def _base_instructions(segments: int, total_length: float, segment_length: float, reference_count: int, has_video: bool = False) -> str:
    ...
    text = (
        ...
        f"(reactions, small movements, repositioning) so the segment keeps filling time instead "
        f"of stopping. Exception: if the segment is built around a spoken line, do not pad it "
        f"with extra mouth- or speech-adjacent business to fill time -- see the dialogue rule "
        f"below; a shorter, cleaner segment beats a longer one that confuses the renderer's "
        f"mouth movement. The LAST new action in a segment MUST land within about the final "
        f"{buffer:.2f} seconds of that segment's own span: late enough to use the available "
        f"time, but never exactly at or after that segment's own end-of-range timestamp, which "
        f"leaves no time to render.\n\n"
        f"If the story includes a specific spoken line (quoted dialogue), treat it as a single, "
        f"tightly bounded event: state it once, verbatim, in quotes, tied to an explicit short "
        f"timestamp window (e.g. \"at 0.50 seconds, she says, '...'\", clearly finishing within a "
        f"couple seconds). Do not describe any other mouth- or speech-adjacent business -- "
        f"sipping a drink, sighing, muttering, extra lip movement -- anywhere else in the "
        f"segment, before or after the line. A downstream renderer reading multiple mouth-"
        f"adjacent cues spread across a segment tends to keep the mouth moving for the whole "
        f"span between them instead of just the actual line, producing garbled, non-verbal "
        f"mouth movement instead of the real words. It is fine, and expected, for a dialogue "
        f"segment to have fewer distinct beats than a purely physical one -- delivering the "
        f"line cleanly takes priority over filling time with unrelated action.\n\n"
    )
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "preset": (preset_names, {"default": default_preset, "tooltip": "Which MiniMax H3 mode/format to write each segment for. 'Custom' reads its system prompt from the custom_system_prompt socket instead (falls back to this default preset if that's left empty). Add more presets in lc_vision_moviemaker_presets.json."}),
    ...
    def run(
        ...
        if preset == CUSTOM_PRESET_LABEL:
            ...
            if not preset_system_text:
                fallback_entry = presets.get(FALLBACK_PRESET)
                if not fallback_entry:
                    raise ValueError(
                        f"[LC Vision] preset is 'Custom' with no custom_system_prompt, and the "
                        f"fallback preset '{FALLBACK_PRESET}' isn't in lc_vision_moviemaker_presets.json "
                        f"-- provide a system prompt, pick a preset, or restore that preset."
                    )
                print(f"[LC Vision] preset is 'Custom' but custom_system_prompt is empty -- falling back to '{FALLBACK_PRESET}'.")
                preset_system_text = fallback_entry["system_prompt"]
        else:
